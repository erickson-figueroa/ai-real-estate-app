...
image_save_path = os.path.join("../server", "images")
...
# New function to scrape and save images based on MLS number
    ...
    for api_url in api_urls:
        ...
        for article in soup.find_all('article', class_='property'):
            try:
                ...
                if img_tag and mls_num:
                    img_url = img_tag['src']
                    img_data = requests.get(img_url).content
                    
                    # Save the image to the specified directory
                    img_filename = os.path.join(image_save_path, f"{mls_num}.png")
                    with open(img_filename, 'wb') as img_file:
                        img_file.write(img_data)
                    print(f"Saved image for MLS# {mls_num} as {img_filename}")
                else:
                    print(f"Skipping image for MLS# {mls_num} due to missing data")
                    
            except Exception as e:
                print(f"Error processing image for MLS# {mls_num}: {e}")


if __name__ == "__main__":
    scrape_listings()
    avg_price = get_average_price()
    scrape_and_save_images()  # Call the function to scrape and save images
    if avg_price:
